...
const planeIcons = [...Array(24).keys()].map((n) =>
...
L.tileLayer(
  "https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}",
  {
    ...
    zoomOffset: -1,
    accessToken:
      "pk.eyJ1IjoiZ3V5cm95c2UiLCJhIjoiY2wyZXhjdXNkMDQ3NjNra2ptajhlN3J3OSJ9.y3JcnAPeRMLCLifILS8t0Q",
  }
).addTo(map);
...
ws.onopen = (event) => {
  ws.onmessage = (event) => {
    ...
    if (!marker && latitude !== undefined && longitude !== undefined) {
      marker = L.marker([latitude, longitude], { icon: planeIcons[0] }).addTo(
        map
      );
      marker.bindPopup();
      marker
        .bindTooltip(callsign ?? icaoId, {
          permanent: true,
          direction: "right",
          offset: L.point(12, 0),
    ...
    if (marker) {
      ...
      if (callsign !== undefined) marker.getTooltip().setContent(callsign);
